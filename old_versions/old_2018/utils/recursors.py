...
class RecursorRoot():
    ...
    def get_coefs(self):
        coefs = [(coef_label, coef_value)
                 for coef_label, coef_value in self.coefs.items()]
        return coefs
...
class SingleSmooth(RecursorRoot):
    ...
    def prime_recurse(self, reprime=False):
        ...
        self.buffer['level_balance'] = 1 / \
            (1 + pt.exp(-self.coefs['level_balance']))
...
class DoubleSmooth(RecursorRoot):
    ...
    def prime_recurse(self, reprime=False):
        ...
        self.buffer['level_balance'] = 1 / \
            (1 + pt.exp(-self.coefs['level_balance']))
        self.buffer['trend_balance'] = 1 / \
            (1 + pt.exp(-self.coefs['trend_balance']))
    ...
    def recurse_row(self, row_tensor):
        smooth_level = (
            ...
            self.buffer['level_balance'] *
            (self.buffer['level'][-1] - self.buffer['trend'][-1])
        )

        smooth_trend = (
            (1 - self.buffer['trend_balance']) *
            (smooth_level - self.buffer['level_balance'])
        ) + (
    ...
    def build_recurse_tensor(self):
        recurse_tensor = pt.cat([
            pt.cat([
                pt.stack(self.buffer[return_type][1:], dim=0) +
                (horizon * pt.stack(self.buffer[return_type][1:], dim=0))
