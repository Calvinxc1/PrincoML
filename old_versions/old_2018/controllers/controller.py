...
class Controller():
    ...
    def add_link(self, cluster, link_cluster_name, link_type, **kwargs):
        ...
        if link_type == 'output':
            ...
            raise Exception(
                '%s is not a valid link type, use "input" or "output"' % link_type)

    def init_clusters(self, reinit=False):
        for cluster in self.clusters.values():
            cluster.init_cluster(reinit=reinit)
    ...
    def learning_iter(self, epoc, t):
        ...
        for loss_name, loss_value in losses.items():
            ...
            self.loss_record[loss_name].append(
    ...
    def prime_clusters(self, reprime=False):
        for cluster in self.clusters.values():
            cluster.prime_cluster(reprime=reprime)

    def deprime_clusters(self):
        for cluster in self.clusters.values():
            cluster.deprime_cluster()
    ...
    def plot_losses(self, figsize=(16, 10)):
        ...
        plt.axvline(x=self.best_iter,
                    label='best validation loss', color='red')
    ...
    def get_outputs(self, data_override=None):
        if data_override is not None:
            for cluster_name, cluster in self.clusters.items():
                cluster.prime_cluster(
                    data_override=data_override.get(cluster_name, None))
        else:
            self.prime_clusters()
        ...
        for cluster_name, cluster in self.clusters.items():
            if type(cluster).__name__ != 'DataCluster':
                outputs[cluster_name] = cluster.get_output_tensor(
