...
class RootCluster():
    ...
    def get_link_idx(self, cluster_name, link_type):
        link_names = [
            link_clusters['cluster'].name for link_clusters in self.links[link_type]]
    ...
    def add_link(self, cluster, link_type, **kwargs):
        if link_type not in self.links.keys():
            raise Exception('%s: Link type %s not a valid link type' %
                            (self.name, link_type))

        if self.get_link_idx(cluster.name, link_type) is not None:
            raise Exception('%s: Cluster %s already present in %s link' % (
                self.name, cluster.name, link_type))
    ...
    def init_cluster(self, reinit=False, **kwargs):
        ...
        if self.init & (not reinit):
            raise Exception(
                '%s: Cluster already initialized and reinit is False' % self.name)
    ...
    def prime_cluster(self, reprime=False, **kwargs):
        ...
        if self.primed & (not reprime):
            raise Exception(
                '%s: Cluster already primed and reprime is False' % self.name)
    ...
    def get_input_count(self):
        input_count = sum([link_item['cluster'].get_output_count(self)
    ...
    def get_input_tensor(self):
        self.buffers['input_tensor'] = self.buffers.get(
            'input_tensor',
            pt.cat([link_item['cluster'].get_output_tensor(self)
                    for link_item in self.links['input']], dim=1)
        )
