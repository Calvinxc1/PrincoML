...
class MergeCluster(Root):
    ...
    def get_input_count(self):
        input_items = [link_item['cluster'].get_output_count(
            self) for link_item in self.links['input']]
    ...
    def get_input_tensor(self):
        self.buffers['input_tensor'] = self.buffers.get(
            'input_tensor',
            pt.stack([link_item['cluster'].get_output_tensor(self)
                      for link_item in self.links['input']], dim=2)
        )
